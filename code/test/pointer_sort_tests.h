...
do_pointer_sort_test(size_t nelem)
{
    ...
    if (native_first)
    {
        ...
        //
        auto    mm1_sort_nat = mismatch(pnat_begin, pnat_end, cbegin(sorted_data), cend(sorted_data));
        ...
        //
        sw.start();
        sort(psyn_begin, psyn_end);
        sw.stop();
        el_syn = sw.elapsed_nsec();

        //- Verify that the newly-sorted buffer matches the sorted reference vector.
        //
        auto    mm2_sort_syn = mismatch(psyn_begin, psyn_end, cbegin(sorted_data), cend(sorted_data));
        CHECK(mm2_sort_syn.first == psyn_end);
        auto    mm2_sort_nat = mismatch(pnat_begin, pnat_end, cbegin(sorted_data), cend(sorted_data));
